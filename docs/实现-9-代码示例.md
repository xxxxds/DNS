...
for (ResourceRecord answer : responseMessage.getAnswers()) {
    ...
    if (answer.getType() == 1) { // A Record
        try {
            // 解析RDATA
            InetAddress ip = InetAddress.getByAddress(answer.getRdata());
...
这个例子展示了如何在`Main.java`中组装并启动所有组件。
...
public static void main(String[] args) {
    ...
    QueryCoordinator coordinator = new QueryCoordinator(parser, builder, resolver);
